/******************************************************************************
 *  dcanimator/src/dcanimator/timeline.c
 *
 */
#define _GNU_SOURCE
...
char *timeline_init(
    ...
    char    *_fg_red = config_get(&app->config, "timeline_fg_0");
    char    *_fg_green = config_get(&app->config, "timeline_fg_1");
    char    *_fg_blue = config_get(&app->config, "timeline_fg_2");

    char    *_bg_red = config_get(&app->config, "timeline_bg_0");
    char    *_bg_green = config_get(&app->config, "timeline_bg_1");
    char    *_bg_blue = config_get(&app->config, "timeline_bg_2");

    AREA    _area = {
        .top = (ui_components->term_y - 8),
        .left = 1,
    ...
    ui_components->component[UI_TIMELINE] = component_new("Timeline", UI_TIMELINE, &_area, &_fg, &_bg, (CMP_F_VISIBLE));
