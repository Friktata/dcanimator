...
char *layers_init(
    ...
    char    *_fg_red = config_get(&app->config, "layers_fg_0");
    char    *_fg_green = config_get(&app->config, "layers_fg_1");
    char    *_fg_blue = config_get(&app->config, "layers_fg_2");

    char    *_bg_red = config_get(&app->config, "layers_bg_0");
    char    *_bg_green = config_get(&app->config, "layers_bg_1");
    char    *_bg_blue = config_get(&app->config, "layers_bg_2");

    AREA    _area = {
        .top = 2,
        .left = (ui_components->term_x - 12),
        .width = 11,
    ...
    ui_components->component[UI_LAYERS] = component_new("Layers", 2, &_area, &_fg, &_bg, 0);
